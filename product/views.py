...
class ListSearchView(APIView):
    ...
    def post(self, request, format = None):
        ...
        search = data['search']
        if len(search)== 0:
            search_results=Joyas.objects.order_by('-date_created').filter(sold=False)
        else:
            search_results=Joyas.objects.filter(sold=False & Q(description__icontains=search) | Q(name__icontains=search))
        if category_id == 0:
        ...
        category = Category.objects.get(id=category_id)
        if category.parent:
            search_results = search_results.order_by('-date_created').filter(category=category, sold=False)
        else:
            if not Category.objects.filter(parent=category).exists():
                search_results = search_results.order_by('-date_created').filter(category=category, sold=False)
            else:
                ...
                for cat in categories:
                    filtered_categories.append(cat)
                filtered_categories=tuple(filtered_categories)
                search_results = search_results.order_by('-date_created').filter(sold=False, category__in=filtered_categories)
        search_results = JoyasSerializer(search_results, many=True)
        return Response({'search_products': search_results.data}, status=status.HTTP_200_OK)         
...
class ListBySearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        order = data['order']
        if category_id == 0:
            products_results = Joyas.objects.filter(sold=False)
        elif not Category.objects.filter(id=category_id).exists():
            return Response(
                {'error': 'La categoria no existe'},
                status=status.HTTP_404_NOT_FOUND)
            ...
            category = Category.objects.get(id=category_id)
            if category.parent:
                products_results = Joyas.objects.filter(category=category, sold=False)
            else:
                if not Category.objects.filter(parent=category).exists():
                    products_results = Joyas.objects.filter(category=category, sold=False)
                else:
                    ...
                    for cat in categories:
                        filtered_categories.append(cat)
                    filtered_categories = tuple(filtered_categories)
                    products_results = Joyas.objects.filter(category__in=filtered_categories, sold=False)
        # filtrar por precio
