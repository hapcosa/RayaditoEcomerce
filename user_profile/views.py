    ...
    def delete(self, request, format=None):
        ...
        try:
            ...
            return Response({'error': 'Error al remover direccion'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
class GetUserProfileViewDetail(APIView):
    def get(self, request, ProfileID, format=None):
        try:
            id=int(ProfileID)
        except:
            return Response({'error': 'Error de tipo de dato, no se pudo acceder ala dirección'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        try:
            user = self.request.user
            user_profile = UserProfile.objects.filter(user=user, id=ProfileID)
            user_profile = UserProfileSerializer(user_profile)
            ...
            return Response(
                {'error': 'Error: No se pudo identificar la direccion'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR)